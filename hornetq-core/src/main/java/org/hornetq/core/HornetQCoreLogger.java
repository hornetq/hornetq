...
public interface HornetQCoreLogger extends BasicLogger
{
   /**
    * The default logger.
    */
   HornetQCoreLogger LOGGER = Logger.getMessageLogger(HornetQCoreLogger.class, HornetQCoreLogger.class.getPackage().getName());
   ...
   @LogMessage(level = Logger.Level.WARN)
   @Message(id = 112003, value = "Timed out waiting for pool to terminate {0}. Interrupting all its threads!", format
   ...
   @LogMessage(level = Logger.Level.WARN)
   @Message(id = 112005, value = "Must specify an address for each divert. This one will not be deployed.", format =
         Message.Format.MESSAGE_FORMAT)
   void divertWithNoAddress();

   @LogMessage(level = Logger.Level.WARN)
   @Message(id = 112006, value = "Must specify a forwarding address for each divert. This one will not be deployed.",
         format = Message.Format.MESSAGE_FORMAT)
   void divertWithNoForwardingAddress();

   @LogMessage(level = Logger.Level.WARN)
   @Message(id = 112007, value = "Binding already exists with name {0}, divert will not be deployed", format =
         Message.Format.MESSAGE_FORMAT)
   ...
   @LogMessage(level = Logger.Level.WARN)
   @Message(id = 112011, value = "Critical IO Error, shutting down the server. code={0}, message={1}", format =
         Message.Format.MESSAGE_FORMAT)
   ...
   @LogMessage(level = Logger.Level.WARN)
   @Message(id = 112022, value = "Waiting {0} milliseconds before next retry. RetryInterval={1} and multiplier={2}",
         format = Message.Format.MESSAGE_FORMAT)
   ...
   @Message(
            id = 112023,
            value = "connector.create or connectorFactory.createConnector should never throw an exception, implementa" +
                  "tion is badly behaved, but we will deal with it anyway."
               , format = Message.Format.MESSAGE_FORMAT)
   ...
   @Message(
            id = 112032,
            value = "I am closing a core ClientSession you left open. Please make sure you close all ClientSessions e" +
                  "xplicitly before letting them go out of scope! {0}",
            format = Message.Format.MESSAGE_FORMAT)
   ...
   @Message(
            id = 112045,
            value = "Closing a Server Locator left open. Please make sure you close all Server Locators explicitly " +
                  "before letting them go out of scope! {0}",
            format = Message.Format.MESSAGE_FORMAT)
   ...
   @Message(id = 112050, value = "There are more than one servers on the network broadcasting the same node id. "
                     ...
                     + "active concurrently with the same node id. This could occur if you have a backup node active " +
         "at the same time as "
   ...
   @Message(id = 112191, value =   "{0}::Remote queue binding {1} has already been bound in the post office. Most likely cause for this is you have a loop " +
                                   "in your cluster due to cluster max-hops being too large or you have multiple " +

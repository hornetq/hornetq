...
public class ClusteredBridgeReconnectTest extends ClusterTestBase
{
         ...
         rules =
            {
               @BMRule
                  (
                     name = "enter",
                     targetClass = "org.hornetq.core.server.cluster.impl.BridgeImpl",
                     targetMethod = "connect",
                     targetLocation = "ENTRY",
                     action = "org.hornetq.byteman.tests.ClusteredBridgeReconnectTest.enterConnect();"
                  ),
               @BMRule
                  (
                     name = "exit",
                     ...
                     targetMethod = "connect",
                     targetLocation = "EXIT",
                     action = "org.hornetq.byteman.tests.ClusteredBridgeReconnectTest.exitConnect();"
                  ),
               @BMRule
                  (
                     name = "send",
                     targetClass = "org.hornetq.core.protocol.core.impl.ChannelImpl",
                     targetMethod = "send(org.hornetq.core.protocol.core.Packet)",
                     targetLocation = "EXIT",
                     action = "org.hornetq.byteman.tests.ClusteredBridgeReconnectTest.send();"
                  )
   ...
   public void testReconnectBridge() throws Exception
   {
      ...
      for (int i = 0; i < NUMBER_OF_MESSAGES; i++)
      {
         ...
         {
            shouldFail = true;
            bridge.getSessionFactory().getConnection().fail(new HornetQException("failed once!"));
         }
      }

...
public class ClusteredGroupingTest extends ClusterTestBase
{
   ...
   {
      SimpleString id = server.getGroupingHandler().getProposal(new SimpleString("id1.queue0")).getClusterName();
      for (HornetQServer qServer : qServers)
      {
         Response proposal = qServer.getGroupingHandler().getProposal(new SimpleString("id1.queue0"));
         if (proposal != null)
         {

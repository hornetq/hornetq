...
public class ConcurrentDeliveryCancelTest extends JMSTestBase
{
         ...
         rules =
            {
               @BMRule
                  (
                     ...
                     action = "org.hornetq.byteman.tests.ConcurrentDeliveryCancelTest.enterCancel();"
                  )
            }
      )
   public void testConcurrentCancels() throws Exception
   {
      ...
      for (int i = 0; i < 100; i++)
      {
         HornetQXAConnectionFactory xacf = (HornetQXAConnectionFactory)HornetQJMSClient.createConnectionFactoryWithHA(JMSFactoryType.XA_CF, new TransportConfiguration(NETTY_CONNECTOR_FACTORY));

         final XAConnection connection = xacf.createXAConnection();
         final XASession theSession = connection.createXASession();
         ((HornetQSession)theSession).getCoreSession().addMetaData("theSession", "true");

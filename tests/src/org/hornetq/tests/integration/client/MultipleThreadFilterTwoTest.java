...
/**
 * Multiple Threads producing Messages, with Multiple Consumers on a single Queue consuming with different filters.
 * This is similar to MultipleThreadFilterOneTest but it uses a single queue
 *
 * @author clebertsuconic
 *
 *
 */
public class MultipleThreadFilterTwoTest extends ServiceTestBase
{
   ...
   class SomeConsumer extends Thread
   {
      ...
      public SomeConsumer(int nr) throws Exception
      {
         ...
         if (nr == 0)
         {
            // Since all share the same queue, only the first consumer will need to create the queue
            consumerSession.createQueue(ADDRESS, "Q", true);
         }
         consumer = consumerSession.createConsumer("Q", "prodNR=" + nr);

...
import org.jboss.test.messaging.jms.perf.data.Measurement;

/**
 * @author <a href="mailto:tim.fox@jboss.com">Tim Fox</a>
 */
public class PerfRunner extends TestCase
{
   private static final Logger log = Logger.getLogger(PerfRunner.class);   
   
   protected String jndiServerURL;
   ...
   public void setUp()
   {
      ...
      jndiServerURL = System.getProperty("perf.jndiServerURL", "jnp://terra:1099");
      queueName = System.getProperty("perf.queueName", "queue/perfTestQueue");
      topicName = System.getProperty("perf.topicName", "topic/perfTestTopic");
      subscriptionName = System.getProperty("perf.subscriptionName", "perfTestSub");      
      queueNamePrefix = System.getProperty("perf.queueNamePrefix", "queue/perfTestQueue");
      topicNamePrefix = System.getProperty("perf.topicNamePrefix", "topic/perfTestTopic");
      numMessages = new Integer(System.getProperty("perf.numMessages", "200")).intValue();
      numWarmupMessages = new Integer(System.getProperty("perf.numWarmupMessages", "100")).intValue();
      providerName = System.getProperty("perf.providerName", "JBossMessaging");
      String dbURL = System.getProperty("perf.dbURL", "jdbc:hsqldb:hsql://localhost:7776");
      scaleNumber = new Integer(System.getProperty("perf.scaleNumber", "8")).intValue();
      standardMessageSize = new Integer(System.getProperty("perf.standardMessageSize", "1024")).intValue();
      initialPause = new Integer(System.getProperty("perf.initialPause", "5000")).intValue();
      clientID = System.getProperty("perf.clientID", "perfTestClientID");
      numSlaves = new Integer(System.getProperty("perf.numSlaves", "2")).intValue();

      slaveURLs = new String[numSlaves];
      for (int i = 0; i < numSlaves; i++)
      {
         slaveURLs[i] = System.getProperty("perf.slaveURL" + (i + 1), "socket://localhost:1234/?socketTimeout=0");
      }
   ...
   {
      return new SenderJob(slaveURLs[0], jndiServerURL, destName, 1,
   ...
   {
      return new ReceiverJob(slaveURLs[0], jndiServerURL, destName, 1,
            1, false, 0, numMessages,
   ...
   {
      return new BrowserJob(slaveURLs[0], jndiServerURL, destName, 1,
            1, numMessages, null);
   }
   
   protected DrainJob createDefaultDrainJob(String destName)
   {
      return new DrainJob(slaveURLs[0], jndiServerURL, destName, null, null);
   }
   
   protected FillJob createDefaultFillJob(String destName, int numMessages)
   {
      return new FillJob(slaveURLs[0], jndiServerURL, destName, numMessages, standardMessageSize, new BytesMessageMessageFactory(),

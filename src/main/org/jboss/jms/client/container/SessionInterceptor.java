      ...
      String methodName = mi.getMethod().getName();
			...
			if (acknowledgmentMode != Session.CLIENT_ACKNOWLEDGE)
         {
			...
			else
			{
				...
				while (iter.hasNext())
				{
					...
					getDelegate(mi).acknowledge(ackInfo.messageID, ackInfo.destination, ackInfo.receiverID);
				}
				unacked.clear();
				return null;
			}
		...
		{		
			...
			if (Xid != null)
			{
				...
				getDelegate(mi).acknowledge(messageID, destination, receiverID);
			}
			...
			else if (acknowledgmentMode == Session.CLIENT_ACKNOWLEDGE)
			{
				...
				getUnacknowledged(invocation).add(new AckInfo(messageID, destination, receiverID));

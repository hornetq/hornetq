   ...
   public boolean handle(Routable m)
   {
      try
      {
         ...
         if (NACKING.equals(state))
         {
            ...
            log.info("Receiver [" + id + "] is NACKing the message");
            return false;
         }
         // Handling receiver
         if (m == null)
         {
            log.info("Receiver [" + id + "] is NACKing a null message");
            return false;
         }
         messages.add(m);
         log.info("Receiver [" + id + "] is ACKing the message");
         return true;
      }
      finally
      {

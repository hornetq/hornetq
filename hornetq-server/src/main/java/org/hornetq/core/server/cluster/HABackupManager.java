...
* */
public class HABackupManager implements HornetQComponent
{
   ...
   public HABackupManager(HornetQServer server, ExecutorFactory executorFactory, ScheduledExecutorService scheduledExecutor, NodeManager nodeManager,
   ...
   private final class StaticBackupConnector extends BackupConnector
   {
      ...
      public ServerLocatorInternal createServerLocator(Topology topology)
      {
         if (tcConfigs != null && tcConfigs.length > 0)
         {
            if (HornetQServerLogger.LOGGER.isDebugEnabled())
            {
               HornetQServerLogger.LOGGER.debug(HABackupManager.this + "Creating a serverLocator for " + Arrays.toString(tcConfigs));
            }

...
public interface HornetQServerLogger extends BasicLogger
{
   ...
   @LogMessage(level = Logger.Level.WARN)
   @Message(id = 222181, value = "Unable to export messages", format = Message.Format.MESSAGE_FORMAT)
   void failedToExport(@Cause Throwable e);

   @LogMessage(level = Logger.Level.WARN)
   @Message(id = 222182, value = "Missing cluster-configuration for exportation-clustername {0}", format = Message.Format.MESSAGE_FORMAT)
   void missingClusterConfigForExport(String exportationCluster);
